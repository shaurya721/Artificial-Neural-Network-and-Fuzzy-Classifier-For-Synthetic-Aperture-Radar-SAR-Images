...
%disp('<strong>PRANAV UPADHYAY-1BG21EC066</strong>');
%disp('<strong>SURAJ KUMAR-1BG21EC095</strong>');
disp('<strong>----------------------------------</strong>');
pause(1);
disp('<strong>**************************************************************************************************</strong>');
...
imghh = imread('C:\5th sem\ML SAR\SYNTHETIC APERTURE RADAR (SAR)\HH_polarized3.tif');
imghv = imread('C:\5th sem\ML SAR\SYNTHETIC APERTURE RADAR (SAR)\HV_polarized3.tif');
imgvh = imread('C:\5th sem\ML SAR\SYNTHETIC APERTURE RADAR (SAR)\VH_polarized3.tif');
imgvv = imread('C:\5th sem\ML SAR\SYNTHETIC APERTURE RADAR (SAR)\VV_polarized3.tif');

% Define window size for Lee filter
...
% Check the size and number of color channels
disp(size(imghh));
disp(size(imghv));
disp(size(imgvh));
disp(size(imgvv));

% % Apply lee filter process with adjusted window size
% imghh(:,:,1) = lee_filter_process(imghh(:,:,1), window_size);
% imghv(:,:,1) = lee_filter_process(imghv(:,:,1), window_size);
% imgvh(:,:,1) = lee_filter_process(imgvh(:,:,1), window_size);
% imgvv(:,:,1) = lee_filter_process(imgvv(:,:,1), window_size);

% Pauli decomposition apply to get RGB image and gray image
% Convert uint8 datatype
% Remove the attempt to access non-existent color channels
imghh1 = abs(imghh + imghh) / 2;
imgvh1 = abs(imgvh + imgvh) / 2;
imghv1 = abs(imghv + imghv) / 2;
imgvv1 = abs(imgvv + imgvv) / 2;
...
original_images = {imghh, imghv, imgvh, imgvv};

% Define augmentation parameters
rotation_angles = [-30, -15, 0, 15, 30];
scaling_factors = [0.9, 1, 1.1];
    ...
    for angle = rotation_angles
        for scale = scaling_factors
            for k = 1:length(flips)
                flip_type = flips{k}; % Get the flip type as a character vector
                
                % Rotate image
                rotated_img = imrotate(img, angle, 'bilinear', 'crop');
                
                % Scale image
                scaled_img = imresize(rotated_img, scale);
                
                % Flip image
                switch flip_type
                    case 'horizontal'
                        flipped_img = flip(scaled_img, 2);
                    case 'vertical'
                        flipped_img = flip(scaled_img, 1);
                    otherwise
                        flipped_img = scaled_img;
                end
                
                % Add augmented image to dataset
                augmented_images{end+1} = flipped_img;
            end
        end
    end
end

% Now augmented_images contains the original and augmented images
...
% Define the lee_filter_process function at the end of the script
function filtered_image = lee_filter_process(image, window_size)
    [rows, cols] = size(image);
    filtered_image = zeros(rows, cols);

    for i = 1:rows
        for j = 1:cols
            % Define the neighborhood for the current pixel
            ...
            % Apply Lee filter to the local window
            filtered_pixel = mean(local_window(:));
